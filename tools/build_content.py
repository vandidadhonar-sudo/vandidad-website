...
def render_home_latest(existing: str, articles: list[Article]) -> str:
    """Put the newest articles on the homepage, and keep them there.

    WHY THIS EXISTS
    ---------------
    Search Console said of a freshly published article: "No referring
    sitemaps detected. Referring page: None detected." Both halves were true
    in the way that matters. The path from an indexed page to a new article
    ran homepage → /hamzad → article, and /hamzad is not indexed yet, so the
    chain was broken in the middle and every new article was an island.

    The homepage is the one page Google has indexed, and it is at position 1
    for the owner's name. An article linked from it is one hop from the
    strongest page on the domain, on the page a crawler returns to most
    often. That is the difference between an article being found in days and
    being found whenever /hamzad is eventually crawled.

    It also ends the part the owner was right to object to: without this,
    every article needs someone to open Search Console and press Request
    Indexing by hand, which is not automation, it is a chore with a
    deadline. This regenerates on every publish, so the newest article is
    linked the moment it exists and nobody has to remember anything.

    WHY IT REWRITES BETWEEN MARKERS RATHER THAN BUILDING THE PAGE
    -------------------------------------------------------------
    index.html is hand-written and carries the hero video, the language
    switcher and the conversation demo. Generating it would put all of that
    at risk for a list of links. This replaces exactly the text between two
    comments and cannot touch a byte outside them — the same approach that
    keeps /about's person section generated without owning the page.

    The block carries no data-i18n attributes on purpose: the language
    switcher rewrites the innerHTML of anything that has one, and these
    articles are Persian whichever language the interface is set to.
    """
    if LATEST_START not in existing or LATEST_END not in existing:
        return existing

    # Liveness is the folder, not a front-matter field — a queued article
    # carries collection "queue" until release_next.py moves it. Reading a
    # date instead is how an audit once counted thirty-seven articles as
    # published when nine were.
    live = [a for a in articles if a.collection != "queue"]
    newest = sorted(live, key=lambda a: a.published, reverse=True)[:6]

    rows = "\n".join(
        f'      <li><a href="/{a.collection}/{a.slug}">{html.escape(a.title)}</a></li>'
        for a in newest
    )
    block = (
        LATEST_START + "\n"
        "  <!-- Generated by tools/build_content.py. Do not edit by hand:\n"
        "       the next publish overwrites everything between the markers. -->\n"
        '  <section id="latest" aria-labelledby="latest-h">\n'
        '    <h2 id="latest-h">تازه‌ترین نوشته‌ها</h2>\n'
        '    <ul class="latest-list">\n'
        f"{rows}\n"
        "    </ul>\n"
        '    <a class="latest-all" href="/hamzad">همهٔ نوشته‌ها</a>\n'
        "  </section>\n"
        "  " + LATEST_END
    )
    head, _, rest = existing.partition(LATEST_START)
...
def main() -> int:
    ...
    if home.exists():
        outputs.append(
            ("index.html", render_home_latest(home.read_text("utf-8"),
                                              everything))
        )
